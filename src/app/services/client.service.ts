...
// );
  ...
  getClients(req:any): Observable<any> {
    //return of(this.clientsSignal());
    let url = 'https://fefgngtdc1.execute-api.us-east-1.amazonaws.com/prod/details';
    return this.http.post(url,req);
  }

  getClientById(id: string): Observable<ClientProfile | undefined> {
    return of(this.clientsSignal().find(client => client.client_id === id));
  }

  updateClient(id: string, updates: Partial<ClientProfile>): Observable<ClientProfile | null> {
    const clients = this.clientsSignal();
    const index = clients.findIndex(client => client.client_id === id);
  ...
  getClientDeails(req:any): Observable<any>{
    let url = 'https://4uegumbk6f.execute-api.us-east-1.amazonaws.com/prod/insights';
    return this.http.post(url,req);
  }
}
